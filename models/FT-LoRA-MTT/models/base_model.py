...
class BaseModel(ABC):
    ...
    def setup(self, opt):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                finetune_lora = getattr(opt, 'finetune_lora', False)
                if not self.isTrain or opt.continue_train or opt.use_lora or finetune_lora:
                    if opt.use_lora or finetune_lora:
                    ...
                    # finetune_lora: strict=False because the new task's adapter keys
                    # are absent from the checkpoint — they stay at their default init.
                    net.load_state_dict(state_dict, strict=not (opt.use_lora or finetune_lora))
                    
                    # Wrap network with LoRA adapters and freeze backbone
                    if opt.use_lora:
                        self.wrap_lora(net, opt)
                    elif finetune_lora:
                        # Only generators have MultiTaskLoRA — freeze backbone, keep new adapter.
                        # Discriminators have no LoRA adapters; they train fully (FiLM row already
                        # resized above, so learning task conditioning is sufficient).
                        if name.startswith('G'):
                            new_task_idx = opt.tasks.index(opt.finetune_lora)
                            self.freeze_for_lora_finetune(net, new_task_idx=new_task_idx)
    ...
    def freeze_for_lora_finetune(self, net, new_task_idx: int):
        """Freeze everything in *net* except the new task's LoRA adapter pair.

        Intended for networks built with MultiTaskLoRA (e.g. ResnetGeneratorLoRA).
        The adapters at index ``new_task_idx`` inside every MultiTaskLoRA module
        are kept trainable; the shared backbone convs and all existing adapters
        are frozen.  The discriminators are left unchanged (always fully trainable).

        Args:
            net:           A generator network containing MultiTaskLoRA layers.
            new_task_idx:  Index of the new task (== len(pretrained_tasks)).
        """
        new_pfx = (f"lora_As.{new_task_idx}.", f"lora_Bs.{new_task_idx}.")
        trainable = total = 0
        for pname, p in net.named_parameters():
            is_new_adapter = any(pfx in pname for pfx in new_pfx)
            p.requires_grad = is_new_adapter
            total += p.numel()
            if is_new_adapter:
                trainable += p.numel()
        print(
            f"  LoRA finetune | frozen: {(total - trainable) / 1e6:.2f}M  "
            f"trainable (new adapter, task {new_task_idx}): {trainable / 1e3:.1f}K params"
        )
