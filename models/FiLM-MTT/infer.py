"""Standalone inference script for the FiLM multi-task CycleGAN model.

Reads hyperparams.json from the checkpoint to reconstruct the architecture
automatically, then conditions the generator on a specific task.

Usage:
  # Translate a folder of images A -> B for task 'horse2zebra'
  python infer.py --input path/to/images --checkpoint checkpoints/FiLM_MTT_... --task horse2zebra

  # Translate B -> A
  python infer.py --input path/to/images --checkpoint checkpoints/FiLM_MTT_... --task horse2zebra --direction BtoA

  # Pick a specific epoch's weights
  python infer.py --input path/to/images --checkpoint checkpoints/FiLM_MTT_... --task horse2zebra --epoch 50

  # Custom output folder
  python infer.py --input path/to/images --checkpoint checkpoints/FiLM_MTT_... --task horse2zebra --out results/my_run
"""
...
def build_generator(checkpoint_path: str, direction: str, epoch: str,
    ...
    """Build and load a FiLM generator from a checkpoint file."""
    tasks = hp["tasks"]
    num_tasks = len(tasks)
    ...
    net = define_G(
        ...
        ngf=hp.get("ngf", 64),
        netG=hp.get("netG", "resnet_9_blocks_film"),
        ...
        num_tasks=num_tasks,
        film_emb_dim=hp.get("film_emb_dim", 64),
    )
...
def main():
    parser = argparse.ArgumentParser(description="Run inference with a trained FiLM multi-task CycleGAN generator.")
    parser.add_argument("--input",      required=True, help="Path to an image file or folder of images")
    parser.add_argument("--checkpoint", required=True, help="Path to checkpoint folder (e.g. checkpoints/FiLM_MTT_...)")
