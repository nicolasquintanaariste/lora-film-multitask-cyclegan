"""Standalone inference script for the LoRA multi-task CycleGAN model.

Reads hyperparams.json from the checkpoint to reconstruct the architecture
automatically, then conditions the generator on a specific task via its LoRA adapter.

Usage:
  # Translate a folder of images A -> B for task 'horse2zebra'
  python infer.py --input path/to/images --checkpoint checkpoints/LoRA_MTT_... --task horse2zebra

  # Translate B -> A
  python infer.py --input path/to/images --checkpoint checkpoints/LoRA_MTT_... --task horse2zebra --direction BtoA

  # Pick a specific epoch's weights
  python infer.py --input path/to/images --checkpoint checkpoints/LoRA_MTT_... --task horse2zebra --epoch 50

  # Custom output folder
  python infer.py --input path/to/images --checkpoint checkpoints/LoRA_MTT_... --task horse2zebra --out results/my_run
"""
...
def build_generator(checkpoint_path: str, direction: str, epoch: str,
    ...
    """Build and load a LoRA generator from a checkpoint file."""
    tasks = hp["tasks"]
    num_tasks = len(tasks)
...
def main():
    parser = argparse.ArgumentParser(description="Run inference with a trained LoRA multi-task CycleGAN generator.")
    parser.add_argument("--input",      required=True, help="Path to an image file or folder of images")
    parser.add_argument("--checkpoint", required=True, help="Path to checkpoint folder (e.g. checkpoints/LoRA_MTT_...)")
