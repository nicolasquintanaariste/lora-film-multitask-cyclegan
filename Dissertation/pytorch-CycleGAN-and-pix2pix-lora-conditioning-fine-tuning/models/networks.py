...
def define_G(input_nc, output_nc, ngf, netG, norm="batch", use_dropout=False, init_type="normal", init_gain=0.02, num_tasks=None, lora_ranks=4):
    ...
    Parameters:
        ...
        netG (str) -- the architecture's name: resnet_9blocks | resnet_6blocks |
                      c | resnet_6blocks_lora |
                      resnet_9blocks_film | unet_128 | unet_256
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm="batch", init_type="normal", init_gain=0.02, num_tasks=None):
    ...
    elif netD == "basic_film":
        net = NLayerDiscriminatorFiLM(input_nc, ndf, n_layers=3, norm_layer=norm_layer, num_tasks=num_tasks)
    elif netD == "n_layers_film":
        net = NLayerDiscriminatorFiLM(input_nc, ndf, n_layers=n_layers_D, norm_layer=norm_layer, num_tasks=num_tasks)
    elif netD == "pixel_film":
...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input, tid=None):
        """Standard forward"""
...
# FiLM ResNet
class ResnetGeneratorFiLM(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type="reflect",
                num_tasks=None, film_emb_dim=64):
        """Construct a Resnet-based generator
        ...
        """
        assert n_blocks >= 0
        super(ResnetGeneratorFiLM, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
    ...
    def forward(self, input, tid=None):
        ...
        for layer in self.model:
            if isinstance(layer, ResnetBlockFiLM):
                input = layer(input, tid)
            else:
                input = layer(input)
...
class ResnetBlockFiLM(nn.Module):
    ...
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias, film: nn.Module = None):
        """Initialize the Resnet block
        ...
        Original Resnet paper: https://arxiv.org/pdf/1512.03385.pdf
        """
        super(ResnetBlockFiLM, self).__init__()
        self.conv_block = self.build_conv_block(dim, padding_type, norm_layer, use_dropout, use_bias)
        self.film = film # optional FiLM module, shared across blocks
    ...
    def forward(self, x, tid=None):
        ...
        if self.film is not None and tid is not None:
            out = self.film(out, tid)   # condition conv features before skip
        return x + out
...
class ResnetBlockLoRA(nn.Module):
    """ResNet block where both conv layers are wrapped with MultiTaskLoRA.

    The shared backbone conv weights receive gradients from every task.
    Each task additionally maintains its own low-rank delta (lora_B_i ∘ lora_A_i)
    which is selected at runtime via the task-id `tid`.
    """
    ...
    def forward(self, x, tid=None):
        out = self.pad1(x)
        out = self.conv1(out, tid)
        out = self.norm1(out)
        out = self.relu(out)
        if self.dropout:
            out = self.dropout(out)
        out = self.pad2(out)
        out = self.conv2(out, tid)
        out = self.norm2(out)
        return x + out  # skip connection


class ResnetGeneratorLoRA(nn.Module):
    """ResNet generator with a shared backbone and per-task LoRA adapters.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d,
        ...
        # Per-task LoRA ResNet blocks (bottleneck)
        mult = 2 ** n_downsampling
...
class NLayerDiscriminatorFiLM(nn.Module):
    """PatchGAN discriminator with optional FiLM conditioning."""

    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, num_tasks=None, film_emb_dim=64):
        super(NLayerDiscriminatorFiLM, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        def make_film(channels):
            return FiLM(num_tasks, channels, film_emb_dim) if num_tasks else None

        self.stages.append(nn.Sequential(nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw), nn.LeakyReLU(0.2, True)))
        ...
        for n in range(1, n_layers):
            ...
            self.film_layers.append(make_film(ndf * nf_mult))
        ...
        self.stages.append(nn.Sequential(nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)))
        self.film_layers.append(None)

    def forward(self, x, tid=None):
        for stage, film in zip(self.stages, self.film_layers):
            x = stage(x)
            if film is not None and tid is not None:
                x = film(x, tid)
        return x
...
class PixelDiscriminatorFiLM(nn.Module):
    """1x1 PatchGAN discriminator with optional FiLM conditioning."""

    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d, num_tasks=None, film_emb_dim=64):
        super(PixelDiscriminatorFiLM, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        self.stage0 = nn.Sequential(nn.Conv2d(input_nc, ndf, kernel_size=1, stride=1, padding=0), nn.LeakyReLU(0.2, True))
        self.stage1 = nn.Sequential(
            nn.Conv2d(ndf, ndf * 2, kernel_size=1, stride=1, padding=0, bias=use_bias),
            norm_layer(ndf * 2),
            nn.LeakyReLU(0.2, True),
        )
        self.film1 = make_film(ndf * 2)
        self.stage2 = nn.Conv2d(ndf * 2, 1, kernel_size=1, stride=1, padding=0, bias=use_bias)

    def forward(self, x, tid=None):
        x = self.stage0(x)
        x = self.stage1(x)
        if self.film1 is not None and tid is not None:
            x = self.film1(x, tid)
