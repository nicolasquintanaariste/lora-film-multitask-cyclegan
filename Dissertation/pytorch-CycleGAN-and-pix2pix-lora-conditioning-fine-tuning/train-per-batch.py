...
if __name__ == "__main__":
    ...
    if getattr(opt, 'finetune_lora', None):
        # Only train on the named task.
        # The full --tasks list is still needed so num_tasks in the network
        # architecture matches the pretrained checkpoint + 1 new adapter.
        ft_task = opt.finetune_lora
        if ft_task not in task2id:
            raise ValueError(f"--finetune_lora task '{ft_task}' not found in --tasks {opt.tasks}")
        task_datasets[task2id[ft_task]] = create_dataset(opt, ft_task)
    else:
