...
if __name__ == "__main__":
    ...
    inspect_trainable(model) if opt.use_lora else 0
    #Create datasets (multitask)
    if "film" in opt.netG.lower():
        opt.dataset_mode = "multitask_unaligned"
    opt.task2id = {t: (model.num_tasks - 1 - i) for i, t in enumerate(opt.tasks)} # Enumerate backwards so that new tasks are at the end
    dataset = create_dataset(opt)  # create a dataset given opt.dataset_mode and other options
    dataset_size = len(dataset)  # get the number of images in the dataset.
    print(f"Iters per epoch = {dataset_size}")
    
    # Saving normalised images of datasets for FID evaluation
    fid_evaluator = FIDEvaluator(opt, transforms_, Tensor, f"results/{opt.name}", opt.task2id)
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):
        with timer.track("train/epoch_total"):
            ...
            for i, data in enumerate(dataset):  # inner loop within one epoch
                ...
                tid = data["tid"] if data["tid"] else None
            ...
            if epoch % 1 == 0: #opt.sample_interval == 0:
                with timer.track("sample_images/compute"):
                    sample_images(epoch, opt, model.netG_A, model.netG_B, opt.task2id, image_folder, Tensor)
                    sample_images(epoch, opt, model.netG_A, model.netG_B, opt.task2id, image_folder, Tensor, 42)
