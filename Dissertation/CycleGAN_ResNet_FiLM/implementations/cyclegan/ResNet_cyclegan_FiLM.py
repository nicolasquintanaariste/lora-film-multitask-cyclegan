...
def main():
    ...
    # Base folder
    base_folder = os.path.join("Dissertation", "CycleGAN_ResNet_FiLM")
    ...
    G_AB = ResnetGenerator(opt.channels, opt.channels, num_tasks=num_tasks)
    G_BA = ResnetGenerator(opt.channels, opt.channels, num_tasks=num_tasks)

    D_A = NLayerDiscriminatorFiLM(opt.channels, ndf=64, num_tasks=num_tasks)
    D_B = NLayerDiscriminatorFiLM(opt.channels, ndf=64, num_tasks=num_tasks)
    ...
    if opt.pretrained_model and not opt.lora:
        ...
        init_weights(G_AB, init_type="normal", init_gain=0.02)
        init_weights(G_BA, init_type="normal", init_gain=0.02)
        init_weights(D_A, init_type="normal", init_gain=0.02)
        init_weights(D_B, init_type="normal", init_gain=0.02)
