...
from loss_utils import LossLogger, plot_losses, d_hinge_loss, g_hinge_loss
...
def main():
    ...
    # Base folder
    base_folder = os.path.join("Dissertation", "CycleGAN_Unet_FiLM_Hinge")
    ...
    # Loss criterions. GAN uses hinge in loss_utils
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        with timer.track("train/epoch_total"):
            ...
            for step, task in enumerate(balanced_tasks):
                ...
                loss_GAN_AB = g_hinge_loss(D_B(fake_B, tid))
                loss_GAN_BA = g_hinge_loss(D_A(fake_A, tid))
                ...
                pred_real = D_A(real_A, tid)
                fake_A_ = fake_A_buffers[task].push_and_pop(fake_A)
                pred_fake = D_A(fake_A_.detach(), tid)
                loss_D_A = d_hinge_loss(pred_real, pred_fake)
                ...
                pred_real = D_B(real_B, tid)
                fake_B_ = fake_B_buffers[task].push_and_pop(fake_B)
                pred_fake = D_B(fake_B_.detach(), tid)
                loss_D_B = d_hinge_loss(pred_real, pred_fake)
                ...
                with torch.no_grad():
                    ...
                    dA_fake_mean = D_A(fake_A_.detach(), tid).mean().item()
                    dB_real_mean = D_B(real_B, tid).mean().item()
                    dB_fake_mean = D_B(fake_B_.detach(), tid).mean().item()
